 ...
 */
package com.dearzs.app.widget.viewflow;
...
public class CircleFlowIndicator extends View implements FlowIndicator,
	...
	private static final int SHAPE_ROUND = 0;
	private static final int SHAPE_RECRANGLE = 1;
	...
	public CircleFlowIndicator(Context context, AttributeSet attrs) {
		...
		int inactiveDefaultColor = 0x88FFFFFF;
		// Get a custom inactive color if there is one
		int inactiveColor = a.getColor(
				R.styleable.CircleFlowIndicator_inactiveColor,
				inactiveDefaultColor);

		shapeType = a.getInt(R.styleable.CircleFlowIndicator_shapeType,
				SHAPE_ROUND);
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int iLoop = 0; iLoop < count; iLoop++) {
			switch (shapeType) {
			case SHAPE_RECRANGLE:
				canvas.drawRect(leftPadding + (iLoop * circleSeparation)
						+ centeringOffset - 3 * radius, getPaddingTop(),
						leftPadding + (iLoop * circleSeparation)
								+ centeringOffset + 5 * radius, getPaddingTop()
								+ 2 * radius, mPaintInactive);
				break;
			default:
		...
		case SHAPE_RECRANGLE:
			...
			canvas.drawRect(leftPadding - 3 * radius + cx + centeringOffset,
					getPaddingTop(), leftPadding + 5 * radius + cx
							+ centeringOffset, getPaddingTop() + 2 * radius,
					mPaintActive);
			break;
		default:
