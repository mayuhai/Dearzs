...
import com.dearzs.app.chat.ui.ImageViewActivity;
...
//                        setImageEvent(viewHolder, uuid,context);
    ...
    public String getSummary() {
        return BaseApplication.getInstance().getString(R.string.summary_image);
    }
                ...
                image.getImage(new TIMValueCallBack<byte[]>() {
                    ...
                    public void onSuccess(byte[] bytes) {
                        if (FileUtil.createFile(bytes, uuid + ".jpg", Environment.DIRECTORY_DOWNLOADS)) {
                            Toast.makeText(BaseApplication.getInstance(), BaseApplication.getInstance().getString(R.string.save_succ), Toast.LENGTH_SHORT).show();
                        } else {
                            Toast.makeText(BaseApplication.getInstance(), BaseApplication.getInstance().getString(R.string.save_fail), Toast.LENGTH_SHORT).show();
                        }
                    }
                });
            }
    ...
    private void setImageEvent(final ChatAdapter.ViewHolder viewHolder, final String fileName, final Context context) {
        getBubbleView(viewHolder).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                ImageViewActivity.startIntent(context, fileName);
            }
        });
    }
    ...
    private void navToImageview(final TIMImage image, final Context context) {
        if (FileUtil.isCacheFileExist(image.getUuid())) {
            ImageViewActivity.startIntent(context, image.getUuid());
        } else {
            if (!isDownloading) {
                isDownloading = true;
                image.getImage(new TIMValueCallBack<byte[]>() {
                    ...
                    public void onError(int code, String desc) {//获取图片失败
                        ...
                        //错误码code含义请参见错误码表
                        Log.e(TAG, "getImage failed. code: " + code + " errmsg: " + desc);
                        Toast.makeText(context, BaseApplication.getInstance().getString(R.string.download_fail), Toast.LENGTH_SHORT).show();
                        isDownloading = false;
                    }
                ...
                Toast.makeText(context, BaseApplication.getInstance().getString(R.string.downloading), Toast.LENGTH_SHORT).show();
            }
        }
    }
}
