...
public class ChatInput extends RelativeLayout implements TextWatcher, View.OnClickListener {
    ...
    public static final int VIEW_TYPE_VOICE = VIEW_TYPE_TEXT + 1;
    public static final int VIEW_TYPE_EMO = VIEW_TYPE_VOICE + 1;
    public static final int VIEW_TYPE_MORE = VIEW_TYPE_EMO + 1;
    public static final int VIEW_TYPE_MORE_NO_FACE_VIDEO = VIEW_TYPE_MORE + 1;

    private static final String TAG = "ChatInput";

    private ImageButton btnAdd, btnSend, btnVoice, btnKeyboard, btnEmotion;
    private EditText editText;
    private boolean isHoldVoiceBtn, isEmoticonReady;
    ...
    private LinearLayout faceVideoLayout;
    private final int REQUEST_CODE_ASK_PERMISSIONS = 100;

    //默认只能发生文字
    private int[] mViewTypeArr = {VIEW_TYPE_TEXT};
    ...
    private void initView() {
        ...
        faceVideoLayout = (LinearLayout) findViewById(R.id.btn_face_video);
        faceVideoLayout.setOnClickListener(this);
    ...
    public void onClick(View v) {
        ...
        switch (v.getId()) {
            ...
            case R.id.btn_video:
                ...
                break;
            case R.id.btn_face_video:
                if (activity != null) {
                    chatView.sendFaceVideo();
                }
                break;
    ...
    public void setViewType(int[] viewTypeArr) {
        if (viewTypeArr != null && viewTypeArr.length > 0) {
            this.mViewTypeArr = viewTypeArr;
        }
        if (this.mViewTypeArr == null) {
            this.mViewTypeArr = new int[]{VIEW_TYPE_TEXT};
        }
        setViewVisible();
    }

    private void setViewVisible() {
        if (mViewTypeArr != null) {
            btnSend.setVisibility(GONE);
            btnVoice.setVisibility(GONE);
            btnEmotion.setVisibility(GONE);
            btnAdd.setVisibility(GONE);
            faceVideoLayout.setVisibility(GONE);
            for (int type : mViewTypeArr) {
                switch (type) {
                    case VIEW_TYPE_TEXT:
                    ...
                    case VIEW_TYPE_MORE:
                        ...
                        faceVideoLayout.setVisibility(VISIBLE);
                        break;
                    case VIEW_TYPE_MORE_NO_FACE_VIDEO:
                        btnAdd.setVisibility(VISIBLE);
                        faceVideoLayout.setVisibility(GONE);
                        break;
                }
